...
/** Descriptive File Name
  ...
  @Description
        ...
        The library implements I2C access to the onboard accelerator device and 
...
float fGRangeLSB;   // global variable used to pre-compute the value in g corresponding for each count of the raw value
...
**      The following digital pins are configured as digital outputs: ACL_INT2.
...
**                                     for example 400000 corresponds to 400 kHz
**
**	Return Value:
**
**	Description:
**		This function configures the SPI1 hardware interface of PIC32, according to the provided frequency.
...
**      It detects the timeout error or slave address not acknowledged as error situations.
**          
*/
...
**
**	Return Value:
**      unsigned char   status      Number of read bytes
**                      0xFF        the slave address was not acknowledged by the device.
**                      0xFE        timeout error
**
**	Description:
...
**      It returns the status of the operation (value obtained from ACL_I2C_Write).
**          
*/
...
**
**	Description:
**		This function reads the value from the register specified by its address, 
**      and returns it.
**          
*/
...
**          "0x0D: WHO_AM_I Device ID register" register
**          
*/
...
**          0x0E: XYZ_DATA_CFG register. The function also precomputes the fGRangeLSB 
...
unsigned char ACL_SetRange(unsigned char bRange)
{
    ...
    bVal &= 0xFC;   // mask out the 2 LSB bits
    bVal |= bRange; // set the 2 LSB bits according to the range value
...
**
**	Parameters:
**      unsigned char *rgRawVals     - Pointer to a buffer where received bytes will be placed. 
 *      It will contain the 6 bytes corresponding to the 3 axis:
...
**
**	Description:
**		This function retrieves the accelerator raw values for the three axis. 
**      Each raw value is represented on 12 bits, so 2 bytes correspond to each of raw value:
**      The MSB byte contains the 8 MSB bits, while the LSB byte contains the 4 LSB bits, 
**      padded right with 4 bits of 0.
**      The raw values are obtained by reading the 6 registers starting with "0x01: OUT_X_MSB"
**      
*/
...
**
**	Parameters:
**      unsigned char *rgRawVals     - Pointer to a buffer that contains the 2 bytes that composes a raw value. 
...
**      precomputed every time the range is set.
**      This functions involves float values computing, so avoid using it when performance is an issue.
**      
*/
...
**
**	Description:
**		This function provides the acceleration values for the three axes, expressed in terms of g.
**      The raw values are acquired using ACL_ReadRawValues. Then, for each axis, the 2 bytes raw value 
**      are converted to the value expressed in terms of g.
**      
*/
...
**
**	Description:
**		This functions releases the hardware involved in ACL library: 
